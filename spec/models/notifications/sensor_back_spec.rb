...
RSpec.describe Notifications::SensorBack, type: :model do
      ...
      context 'when a message was stored in data' do
        let(:notification) do
          described_class.new(
            user:,
            notifiable: sensor,
            data: { via: 'flash', message: 'Custom back-online message' }
          )
        end
        ...
        it 'returns the stored message' do
          ...
          expect(notification.message).to eq('Custom back-online message')
        end
      end
      ...
      context 'when no message was stored in data' do
        ...
        it 'returns the default i18n message with the sensor uid' do
          ...
          expect(notification.message).to eq(
            I18n.t('notifications.sensor_back.message', sensor_uid: sensor.uid)
          )
        end
      end
